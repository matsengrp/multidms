...
class Model(eqx.Module):
    ...
    def predict_post_count(
        ...
        for d in data_sets:
            ...
            result[d] = jnp.power(
                2,
                f + jnp.log2(n_v) + jnp.log2(m_wt) - jnp.log2(n_wt),
            )
        return result
...
def fit(
    ...
    model = Model(
        φ={
            d: Latent(data_sets[d], l2reg_β=l2reg_β)
            for d in data_sets
        },
    ...
    hyperparameters_prox = dict(fusionreg=fusionreg, Ps=Ps)

    filter_spec = Model(φ=True, α=False)
    ...
    for iter in range(maxiter):
        ...
        model_φ, state_φ = opt_φ.run(
            model_φ, hyperparameters_prox, model_α, data_sets, Ps, **kwargs
        )
        model = eqx.combine(model_φ, model_α)
        obj_traj.append(_objective(model, data_sets, fusionreg=fusionreg, **kwargs))
        print(f"  φ block:")
        ...
        for d in data_sets:
            if d != reference_condition:
                ...
                print(f"    {d} shift sparsity={(shifts == 0).sum() / len(shifts):.2%}")
...
def _objective_smooth(model, data_sets, l2reg_α=0.0, l2reg_β=0.0):
    ...
    ridge_beta = sum((model.φ[d].β ** 2).sum() for d in model.φ)
    return loss / n + l2reg_α * ridge_α + l2reg_β * ridge_beta
